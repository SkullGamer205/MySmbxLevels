...
void main()
{
    ...
    vec3 diff = (abs(c - t) + abs(c - b) + abs(c - l) + abs(c - r)) / 1.0;
    ...
    if(any(greaterThan(diff, delta)))
        gl_FragColor += length(1.0 - c);

    vec3 light = texture2D(u_light_buffer, v_fbcoord).rgb;
    gl_FragColor.rgb *= (light.r + light.g + light.b) / 3.0;
}
