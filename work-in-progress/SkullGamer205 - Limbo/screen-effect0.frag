...
void main()
{
    ...
    vec3 diff = (abs(c - t) + abs(c - b) + abs(c - l) + abs(c - r)) / 1.5;

--    gl_FragColor = vec4(0.0, 0.0 * sin((v_texcoord.y * 32.0 + u_clock + 0.5 * cos(u_clock * 3.14)) * 3.14 * length(texture2D(u_texture, v_texcoord).rgb)) + 0.0, 0.0, 1.0);
    gl_FragColor = vec4(0.0, 0.0 * sin((v_fbcoord.y / u_framebuffer_pixsize.y / 4.0 + u_clock * 4.0 + 0.2 * cos(u_clock * 6.28318530718 * 5.0)) * 6.28318530718) + 0.0, 0.0, 1.0);

    if(any(greaterThan(diff, delta)))
    	gl_FragColor += length(texture2D(u_texture, v_texcoord).rgb * (1.0 - c));
}
